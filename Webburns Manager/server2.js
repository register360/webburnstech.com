...
// Firebase Configuration (for Auth and Realtime DB only)
...
const uploadToGoogleDrive = async (fileBuffer, fileName, mimeType, folderId = googleDriveConfig.folderId) => {
  try {
    const fileMetadata = {
      name: fileName,
      parents: [folderId]
...
const createFolderInGoogleDrive = async (folderName, parentFolderId = googleDriveConfig.folderId) => {
  try {
    const fileMetadata = {
      name: folderName,
...
// Create project - UPDATED FOR GOOGLE DRIVE
...
// File Upload for Chat - UPDATED FOR GOOGLE DRIVE
app.post('/api/projects/:projectId/upload', authenticateToken, upload.single('file'), async (req, res) => {
  try {
    ...
    res.json({ url: uploadResult.downloadUrl, message: 'File uploaded successfully' });
  } catch (error) {
...
app.get('/api/admin/drive-stats', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    console.error('Google Drive stats error:', error);
    res.status(500).json({ error: 'Failed to get Google Drive stats' });
  }
});
...
app.delete('/api/files/:fileId', authenticateToken, async (req, res) => {
  try {
    ...
    if (success) {
      // Also remove from MongoDB records
...
io.on('connection', (socket) => {
  ...
  // Commit code changes - UPDATED FOR GOOGLE DRIVE
  socket.on('commit-code', async (data) => {
    ...
    if (codeSession) {
      ...
      try {
        const fileName = `code-backup-${projectId}-${Date.now()}.${codeSession.language}`;
        const fileBuffer = Buffer.from(codeSession.content, 'utf-8');
        ...
        console.error('Failed to backup code to Google Drive:', error);
        // Don't fail the commit if backup fails
      }
...
app.get('/api/health', async (req, res) => {
  ...
  try {
    ...
    healthReport.checks.googleDrive = 'connected';
  } catch (error) {
    healthReport.checks.googleDrive = 'error';
    healthReport.status = 'degraded';
  }
...
server.listen(PORT, () => {
  ...
  console.log(`Google Drive storage configured for folder: ${googleDriveConfig.folderId}`);
});
