...
router.post('/', async (req, res) => {
    try {
        ...
        server_1.logger.info(`New contact form submission from: ${email}`);
        res.json({
            success: true,
        ...
        server_1.logger.error('Contact form submission error:', error);
        res.status(500).json({
...
router.get('/', async (req, res) => {
    try {
        ...
        server_1.logger.error('Get contacts error:', error);
...
router.patch('/:id/status', async (req, res) => {
    try {
        ...
        await contact.save();
        server_1.logger.info(`Contact ${id} status updated to: ${status}`);
        res.json({
            success: true,
        ...
        server_1.logger.error('Update contact status error:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
});
// Helper functions
async function sendAdminNotification(contact) {
    try {
      ...
      `
        ...
        server_1.logger.info(`Admin notification sent for contact: ${contact._id}`);
    }
    catch (error) {
        server_1.logger.error('Failed to send admin notification:', error);
    }
}
async function sendAutoReply(contact) {
    try {
      ...
      `
        ...
        server_1.logger.info(`Auto-reply sent to: ${contact.email}`);
    }
    catch (error) {
        server_1.logger.error('Failed to send auto-reply:', error);
    }
}
