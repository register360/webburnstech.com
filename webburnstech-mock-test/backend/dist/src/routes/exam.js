...
router.get('/questions', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        appGlobals_1.logger.error('Get questions error:', error);
...
router.post('/attempts/start', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        await appGlobals_1.redisClient.setEx(sessionKey, 7200, singleSessionKey);
        appGlobals_1.logger.info(`Exam started for user: ${req.user.userId}`);
        res.json({
            success: true,
            attempt: {
                _id: attempt._id,
        ...
        appGlobals_1.logger.error('Start exam error:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
});
router.post('/attempts/:attemptId/save', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        appGlobals_1.logger.error('Save answer error:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
});
router.post('/attempts/:attemptId/cheating-event', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        await attempt.save();
        appGlobals_1.logger.warn(`Cheating event recorded for user ${req.user.userId}: ${type}`);
        res.json({
            success: true,
            warningCount,
        ...
        appGlobals_1.logger.error('Cheating event error:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
});
router.post('/attempts/:attemptId/submit', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        appGlobals_1.logger.error('Submit exam error:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
});
router.get('/attempts/:attemptId', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        appGlobals_1.logger.error('Get attempt error:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
});
async function autoSubmitAttempt(attempt, reason = 'Time up') {
    ...
    await gradeAndSubmitAttempt(attempt);
    appGlobals_1.logger.info(`Exam auto-submitted for user ${attempt.userId}: ${reason}`);
}
async function gradeAndSubmitAttempt(attempt) {
    ...
    attempt.submittedAt = new Date();
    await attempt.save();
    appGlobals_1.logger.info(`Exam graded for user ${attempt.userId}: Score ${score}`);
}
