...
router.get('/applications', auth_1.adminAuthMiddleware, async (req, res) => {
    try {
        ...
        server_1.logger.error('Get applications error:', error);
...
router.post('/applications/:id/accept', auth_1.adminAuthMiddleware, async (req, res) => {
    try {
        ...
        await user.save();
        server_1.logger.info(`Application accepted for user: ${user.email}`);
        res.json({
            success: true,
        ...
        server_1.logger.error('Accept application error:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
});
// Reject application
router.post('/applications/:id/reject', auth_1.adminAuthMiddleware, async (req, res) => {
    try {
        ...
        user.adminNotes = adminNotes;
        await user.save();
        server_1.logger.info(`Application rejected for user: ${user.email}`);
        // Send rejection email would be implemented here
        res.json({
            success: true,
        ...
        server_1.logger.error('Reject application error:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
});
...
router.post('/applications/:id/send-credentials', auth_1.adminAuthMiddleware, async (req, res) => {
    try {
        ...
        await user.save();
        server_1.logger.info(`Credentials sent manually for user: ${user.email}`);
        res.json({
            success: true,
        ...
        server_1.logger.error('Send credentials error:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
});
// Get exam results
router.get('/results', auth_1.adminAuthMiddleware, async (req, res) => {
    try {
        ...
        server_1.logger.error('Get results error:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
});
// Get cheating events
router.get('/cheating-events', auth_1.adminAuthMiddleware, async (req, res) => {
    try {
        ...
        server_1.logger.error('Get cheating events error:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
});
// Add questions
router.post('/questions', auth_1.adminAuthMiddleware, async (req, res) => {
    try {
        ...
        server_1.logger.error('Add questions error:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
});
// Get system stats
router.get('/stats', auth_1.adminAuthMiddleware, async (req, res) => {
    try {
        ...
        server_1.logger.error('Get stats error:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
});
