...
exports.sendCredentialsEmail = exports.sendVerificationPendingEmail = exports.sendOTPEmail = void 0;
...
const sendOTPEmail = async (email, firstName, otp) => {
    try {
      ...
      `
        ...
        appGlobals_1.logger.info(`OTP email sent to: ${email}`);
        return result;
    }
    catch (error) {
        appGlobals_1.logger.error('Error sending OTP email:', error);
        throw error;
    }
};
...
const sendVerificationPendingEmail = async (email, firstName) => {
    try {
      ...
      `
        ...
        appGlobals_1.logger.info(`Verification pending email sent to: ${email}`);
    }
    catch (error) {
        appGlobals_1.logger.error('Error sending verification pending email:', error);
        throw error;
    }
};
exports.sendVerificationPendingEmail = sendVerificationPendingEmail;
const sendCredentialsEmail = async (user) => {
    try {
      ...
      `
        ...
        appGlobals_1.logger.info(`Credentials email sent to: ${user.email}`);
    }
    catch (error) {
        appGlobals_1.logger.error('Error sending credentials email:', error);
        throw error;
    }
};
