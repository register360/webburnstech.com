...
const sendVerificationPendingEmail = async (email, firstName) => {
    try {
      ...
      `
        ...
        server_1.logger.info(`Verification pending email sent to: ${email}`);
    }
    catch (error) {
        server_1.logger.error('Error sending verification pending email:', error);
        throw error;
    }
};
...
const sendCredentialsEmail = async (user) => {
    try {
      ...
      `
        ...
        server_1.logger.info(`Credentials email sent to: ${user.email}`);
    }
    catch (error) {
        server_1.logger.error('Error sending credentials email:', error);
        throw error;
    }
};
