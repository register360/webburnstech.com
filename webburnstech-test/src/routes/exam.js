...
router.get('/questions', authMiddleware, async (req, res) => {
  try {
    ...
    // Shuffle questions
    questions = questions.sort(() => Math.random() - 0.5).slice(0, 75);
...
// Start exam attempt
router.post('/attempts/start', authMiddleware, async (req, res) => {
  try {
...
// Save answer
router.post('/attempts/:attemptId/save', authMiddleware, async (req, res) => {
  try {
    ...
    const existingAnswerIndex = attempt.answers.findIndex(
      (answer) => answer.qId.toString() === qId
    );
